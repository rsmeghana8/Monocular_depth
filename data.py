...
def get_data(batch_size,data = 'nyu_data_zip'):
    ...
    return data, nyc2_train, nyc_test, shape_rgb, shape_depth
...
class BasicAugmentationRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, shape_rgb, shape_depth,is_flip = False, is_addnoise = False, is_erase = False):
        ...
        self.policy = BasicPolicy(color_change_ratio = 0.50, mirror_ratio = 0.5, flip_ratio = 0.0 if not is_flip else 0.2 ,add_noise_peak = 0 if not is_addnoise else 20, erase_ratio=-1.0 if not erase_ratio else 0.5)
